...
module.exports = function(d3_select, d3_functor, d3_mouse, d3_dispatch, accessor_trade, plot, plotMixin, svg_arrow) {  // Injected dependencies
  return function() { // Closure function
    var p = {},  // Container for private, direct access mixed in variables
        ...
        svgArrow = svg_arrow().orient(function(d) { return p.accessor.typ(d) === 'buy' ? 'up' : 'down'; }),
        arrowGenerator;
    ...
    /**
     * Pass through straight to `d3ts.svg.arrow`.
     *
     ...
     *          a corresponding `d3ts.svg.arrow` orient value is returned.
     */
    ...
    function binder() {
      svgArrow.x(function(d) { return p.xScale(p.accessor.t(d)); }).y(y);
    ...
    function findNearest(d, x) {
      ...
      return d.map(function(d,i) { return { d: d, i: i, x: p.xScale(p.accessor.t(d)) }; }).reduce(function(p, c) {
