...
export class SparklineComponent implements OnChanges {
  ...
  public render(): void {
    ...
    if (cfg.data as Ohlcv[]) {
      yExtent = d3.extent(<Ohlcv[]>cfg.data, d => d.close);
      getY = (d: any) => <Ohlcv>d.close;
    } else if (cfg.data as Trade[]) {
      yExtent = d3.extent(<Trade[]>cfg.data, d => d.price);
      getY = (d: any) => <Trade>d.price;
    } else if (cfg.data as Quote[]) {
      yExtent = d3.extent(<Quote[]>cfg.data, d => d.bidPrice);
      getY = (d: any) => <Quote>d.close;
    } else {
      yExtent = d3.extent(<Scalar[]>cfg.data, d => d.value);
      getY = (d: any) => <Scalar>d.close;
    }
