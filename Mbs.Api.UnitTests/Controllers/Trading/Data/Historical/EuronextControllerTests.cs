...
namespace Mbs.Api.UnitTests.Controllers.Trading.Data.Historical
{
    ...
    public class EuronextControllerTests
    {
        ...
        public async Task GetOhlcvAsync_ValidRequest_ListFetched()
        {
            ...
            Assert.IsTrue(objectResult.StatusCode.HasValue,
                "objectResult should have status code");
            Assert.AreEqual(StatusCodes.Status200OK, objectResult.StatusCode.Value,
                "objectResult status code should be 200");
            Assert.IsInstanceOfType(objectResult.Value, typeof(HistoricalDataReply<Ohlcv>),
                "objectResult.Value should be of type HistoricalDataReply<Ohlcv>");

            var result = (HistoricalDataReply<Ohlcv>)objectResult.Value;
            Assert.IsTrue(result.IsDataAdjusted.HasValue,
                "result.IsDataAdjusted should have value");
            Assert.AreEqual(validRequest.AdjustedDataIfPresent, result.IsDataAdjusted.Value,
                "result.IsDataAdjusted should have correct value");

            var list = (List<Ohlcv>)result.Data;
            Assert.AreEqual(fetchedList.Count, list.Count,
                "list should have correct count");
            Assert.AreEqual(fetchedList[0].Typical, list[0].Typical,
                "list should have correct data (0)");
            Assert.AreEqual(fetchedList[1].Typical, list[1].Typical,
        ...
        public async Task GetOhlcvAsync_ValidRequest_ServiceCalledOnce()
        {
            ...
            mockService.Verify(x => x.FetchOhlcvAsync(It.IsAny<HistoricalDataRequest>()), Times.Once);
        }
        ...
        public async Task GetOhlcvAsync_NotFound_ErrorReturned()
        {
            ...
            Assert.IsInstanceOfType(objectResult.Value, typeof(InternalError),
            ...
            Assert.IsTrue(result.Message.Contains("does not exist or is empty", StringComparison.Ordinal),
                "result.Message should have correct value");
            Assert.IsNull(result.Details,
                "result.Details should be null");
        }
        ...
        public async Task GetOhlcvAsync_BadRequest_ErrorReturned()
        {
            ...
            Assert.IsTrue(objectResult.StatusCode.HasValue,
                "objectResult should have status code");
            Assert.AreEqual(StatusCodes.Status400BadRequest, objectResult.StatusCode.Value,
            ...
            Assert.AreEqual(400, result.StatusCode,
                "result.StatusCode should have correct value");
            Assert.IsTrue(result.Message.Contains("Invalid parameters", StringComparison.Ordinal),
                "result.Message should have correct value");
            ...
            Assert.IsTrue(result.Details.ElementAt(0).Message.Contains("EndDate should be greater than the StartDate", StringComparison.Ordinal),
        ...
        public async Task GetOhlcvAsync_BadRequest_ServiceNotCalled()
        {
            ...
            mockService.Verify(x => x.FetchOhlcvAsync(It.IsAny<HistoricalDataRequest>()), Times.Never);
        }
