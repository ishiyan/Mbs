...
namespace EuronextEndofdayJsonParsing
{
    public static class EuronextEndofdayJsonParsingUsingSpan
    {
        ...
        private static bool TryParseDoubleSpan(ReadOnlySpan<char> str, EntryInfo entryInfo, out double value, out int i)
        {
            ...
            /* -- does not understand thousands delimiter (,) and breaks on alpha character --
            ...
            {
                Log.Error(
                    $"{Prefix}: invalid endofday json: invalid [{entryInfo.Name}] [{number.ToString()}] in [{entry.ToString()}] in [{str.ToString()}], skipping.");
                value = double.NaN;
                return false;
            }*/
