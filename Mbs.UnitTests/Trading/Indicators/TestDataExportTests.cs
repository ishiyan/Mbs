...
namespace Mbs.UnitTests.Trading.Indicators
{
    ...
    public class TestDataExportTests
    {
        ...
        public void ExportBb()
        {
            ...
            {
                var comma3 = i == outputs.Count - 1 ? string.Empty : ",";
                ...
                for (int k = 0; k < 1; ++k)
                {
                    var comma2 = k == 0 ? string.Empty : ",";
                    str += "{outputs:[";
                    for (int j = 0; j < outputs[i].Outputs.Length; ++j)
                    {
                        var comma = j == outputs[i].Outputs.Length - 1 ? string.Empty : ",";
        ...
        public void ExportBollingerBandsAndGoetrzel()
        {
            ...
            for (int i = 0; i < outputs.Count; ++i)
            {
                ...
                for (int k = 0; k < lenInd; ++k)
                {
                    ...
                    list.Add("      {");
                    var comma2 = k == lenInd - 1 ? string.Empty : ",";
                    list.Add("        outputs: [");
                    for (int j = 0; j < lenOut; ++j)
                    {
                        var comma = j == lenOut - 1 ? string.Empty : ",";
                        var o = indOut[j];
                        var str = "          ";
                        switch (o)
                        {
