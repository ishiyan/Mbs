...
namespace Mbs.Numerics
{
    ...
    public static partial class SpecialFunctions
    {
        ...
        {
            return x > 0d ? 1 : (((int)Math.Floor(x)) % 2 == 0) ? 1 : -1;
        }
        ...
        public static Complex Gamma(Complex z)
        {
            ...
            if (z.IsReal)
                return new Complex(Gamma(z.Real));
            return Complex.Exp(LnGamma(z));
        }
        ...
        public static double Digamma(double x)
        {
            ...
            if (x < 1d)
                return EvaluateChebyshevSeries(2d * x - 1d, PsiChebyshevSeriesApproximation) - 1d / x;
            return EvaluateChebyshevSeries(2d * x - 3d, PsiChebyshevSeriesApproximation);
        }
        ...
        {
            if (n <= 0)
                return double.NaN; // throw new ArgumentOutOfRangeException("n");
        ...
        {
            if (n >= 0x7fffffff)
                return Digamma(n + 1d) + Constants.EulerGamma;
            return Digamma(n + 1) + Constants.EulerGamma;
        }
        ...
        public static double RegularizedGammaP(double a, double x)
        {
            if (a <= 0d || x < 0d)
                return double.NaN;
            if (x <= 0.5 * a)
                return IncompleteGammaPSeries(a, x);
            return 1d - RegularizedGammaQ(a, x);
        }
        ...
        public static double RegularizedGammaQ(double a, double x)
        {
            ...
            if (a > 100d && Math.Abs(x / a - 1d) < 0.15)
                return IncompleteGammaqLargeAx(a, x);
            return DominantPart(a, x) * a * IncompleteGammaContinuedFraction(a, x);
        }
        ...
        {
            2.5066282746363426,     43.876640844001564,     -29.441488119961249,     2.617298932416928,
            -0.000840299612359108,  0.00042715899834181971, -0.00023054552812795247, 8.4040594453044693E-05,
        ...
        // private const double lanczosG = 3.6998328;
        // private const double lanczosGLarge = 5.5407;
        ...
        private static double[] ComputeCotDerivative(int n)
        {
            ...
            // The next two methods simply implement this machinery. Is there a way we can turn this into a recursion formula so it is O(n) instead of O(n²)?
...
#pragma warning restore SA1203
        ...
        private static double IncompleteGammaQSeries(double a, double x)
        {
            ...
            // p *= a;
            // q = -0.0001280502823881162 + l * (0.00021524167411495098 + l * (0.00058258379592953258 + l * (-0.0012031572077771833 + l * (0.00040091548032820723 + l * (0.00035164778796286945 + l * (-0.00023130362691984929 + l * (8.3338561575585785E-06 + l * (1.6266817250006296E-05 + l * (-1.590651633877681E-06 + l * -2.7557319223985888E-07)))))))));

            // last two assignments are not used, should we do
            // r += q * p;
            // ?
        ...
        private static double InverseRegularizedGamma(double a, double p, double q)
        {
            ...
            else
            {
                ...
                // x = Math.Max(v, 0d); -- assigned but not used
            ...
            do
            {
                ...
                {
                    if (p < 0.5)
                        r = RegularizedGammaP(a, x) - p;
                    else
                        r = q - RegularizedGammaQ(a, x);
